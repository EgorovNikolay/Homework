...
def test_console_success_kwargs(capsys):
    ...
    assert result == 7
    assert (
        "Начало выполнения функции my_function с аргументами (4,) и {'y': 3}"
        in captured.out
    )
    assert "Результат работы функции 7" in captured.out
    assert (
        "Конец выполнения функции my_function с аргументами (4,) и {'y': 3}"
        in captured.out
    )
    assert "my_function ok" in captured.out
...
def test_console_empty_kwargs(capsys):
    ...
    assert result is None
    assert (
        "Начало выполнения функции my_function с аргументами () и {'y': '3'}"
        in captured.out
    )
    assert (
        "Функция: my_function Ошибка: Аргументы должны быть целым или вещественным числом. "
        "Введенные аргументы: (), {'y': '3'}"
    ) in captured.out
